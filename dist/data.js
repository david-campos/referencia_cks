...
const THE_OBJ = (function () {
    ...
    return {
        ...
        "funcs": [{
            ...
            "description": "<p>Devuelve la \"intersección\" de ambos rectángulos, es decir, el rectángulo correspondiente a la superposición de ambos rectángulos. <strong>Cuidado</strong>: si los rectángulos no se intersecan, el rectángulo resultante puede no ser intuitivo.</p><p>Formalmente, lo que esta función hace es coger el máximo de ambos rectángulos para <a href='#rect::left'>left</a> y <a href='#rect::top'>top</a>, y el mínimo de ambos rectángulos para <a href='#rect::right'>right</a> y <a href='#rect::bottom'>bottom</a>.</p><p><tt>r = IntersectRects(a, b)</tt> es, pues, equivalente a <tt class='language-cks'>r.Set(MAX(a.left, b.left), MAX(a.top, b.top), MIN(a.right, b.right), MIN(a.bottom, b.bottom))</tt></p>",
            "description_en": "Returns the \"intersection\" of both rectangles, i.e., the rectangle formed where both rectangles overlap. <strong>Warning</strong>: if the rectangles do not intersect, the return of this function will not be intuitive.</p><p>In reality, this function takes the maximum for <a href='#rect::left'>left</a> and <a href='#rect::top'>top</a> among the rectangles, and the minimum for <a href='#rect::right'>right</a> and <a href='#rect::bottom'>bottom</a>.</p><p><tt>r = IntersectRects(a, b)</tt> is, therefore, equivalent to <tt class='language-cks'>r.Set(MAX(a.left, b.left), MAX(a.top, b.top), MIN(a.right, b.right), MIN(a.bottom, b.bottom))</tt></p>",
            ...
            "description": "<p>Obtiene el entero en la posición indicada.</p> <p>Las posiciones comienzan en 0 (siendo 0 el primer elemento, 1 el segundo, etc). Por ejemplo, para acceder al cuarto elemento de <tt>miArray</tt> podemos poner: <tt>miArray[3]</tt>. Nótese que el último elemento tiene índice <tt class='language-cks'>miArray.size - 1</tt>. </p><p>Nótese también que el operador devuelve una referencia, por lo que el valor puede ser modificado, por ejemplo, con el <a href='#ptr.int::-e-:ptr.int_int'>el operador de asignación para enteros</a>. Modificar el valor de un índice no existente redimensiona el array para incluir ese índice.</p><p>Si el array no contiene el índice dado, el valor retornado es <tt>0</tt>.</p>",
